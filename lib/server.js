...
server.httpServer = http.createServer((req, res) => {
    ...
    const decoder = new StringDecoder('utf8'); //klientas uploadina info
    let buffer = '';
    req.on('data', (data) => {
    ...
    req.on('end', async () => {
        ...
        if (isAPI) {
            const APIroute = trimmedPath.split('/')[1]; //kai klientas nori apsikeisti duomenimis su musu serveriu
